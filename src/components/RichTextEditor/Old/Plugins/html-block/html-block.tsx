// @ts-nocheck
import React, { useState } from 'react'
import { createRoot } from 'react-dom/client'
import { Code } from '../../../../Code'
import { styled } from 'inlines'
import { color } from '../../../../../varsUtilities'

const HtmlInputBlock = (data) => {
  const [html, setHtml] = useState(data.value.html)

  return (
    <Code
      language="html"
      value={html}
      onChange={(v) => {
        setHtml(v)
      }}
      style={{
        background: color('action', 'neutral', 'subtleHover'),
        marginBottom: 16,
      }}
    />
  )
}

export default class HtmlBlock extends React.Component {
  static get toolbox() {
    return {
      title: 'html',
      icon: `<svg width="14" height="14" viewBox="-3 -3 24 24" fill="black"><path fill-rule="evenodd" clip-rule="evenodd" d="M1.00002 3.75C1.00002 2.23122 2.23124 1 3.75002 1H16.25C17.7688 1 19 2.23122 19 3.75V12.748C19 12.7494 19 12.7509 19 12.7524V16.25C19 17.7688 17.7688 19 16.25 19H3.75002C2.23124 19 1.00002 17.7688 1.00002 16.25V12.5893C0.999993 12.5856 0.999993 12.5819 1.00002 12.5782V3.75ZM2.50002 12.9368V16.25C2.50002 16.9404 3.05966 17.5 3.75002 17.5H16.25C16.9404 17.5 17.5 16.9404 17.5 16.25V13.0759L14.4222 10.2033L11.6778 12.0328L13.7906 14.2302C14.0777 14.5287 14.0684 15.0035 13.7699 15.2906C13.4713 15.5777 12.9965 15.5684 12.7094 15.2698L6.94127 9.27098L2.50002 12.9368ZM10.6215 10.9343L7.54065 7.73017C7.26881 7.44746 6.82507 7.42193 6.5226 7.67158L2.50002 10.9918V3.75C2.50002 3.05964 3.05966 2.5 3.75002 2.5H16.25C16.9404 2.5 17.5 3.05964 17.5 3.75V11.0241L15.0118 8.70171C14.7575 8.46438 14.3734 8.43303 14.084 8.62596L10.6215 10.9343Z" fill="black"></path></svg>`,
    }
  }

  constructor({ data }, props) {
    super(props)
    this.data = {
      html: data.html || '',
    }
  }

  render() {
    const container = document.createElement('div')

    createRoot(container).render(<HtmlInputBlock value={this.data} />)

    return container
  }

  //check if savedata is not empty
  validate(savedData) {
    if (!savedData.html.trim()) {
      return false
    }
    return true
  }

  save(blockContent) {
    // console.log('some html?>?', blockContent)

    let inputFieldValue = blockContent.querySelector('textarea').value

    return {
      html: inputFieldValue,
    }
  }
}
